...
class SentimentDatabaseUpdater:
    ...
    def insert_sentiment_record(
        ...
        try:
            ...
            logger.error(
                f"Exception during sentiment record insertion for {ticker}: {str(e)}"
            )
            logger.error(
                f"Data: URL={article_url[:100]}..., score={sentiment_score}, date={published_at}"
            )
    ...
    def process_articles_with_sentiment(
        ...
        if result["status"] == "error":
            ...
            # Use actual rowcount if available; otherwise leave inserted at 0 to avoid misreporting
    ...
    def update_market_data_sentiment(self, ticker: str) -> None:
        """
        Populate market_data.sentiment_score with 5 minute average sentiment from previous cyles new articles else copies most recent score.
        """
        sql = """
    ...
    def update_from_csv_files(
        ...
        sentiment_dir: str = "NLP/sentiment_scores",
    ) -> bool:
        """

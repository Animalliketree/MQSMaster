...
def get_realtime_quote(tickers, api_key, exchange="NASDAQ"):
    """
    Fetch realtime stock quotes from Financial Modeling Prep API.

    Args:
        tickers (list or str): Ticker(s) as a string or list (e.g., "AAPL" or ["AAPL", "MSFT"]).
        api_key (str): Your Financial Modeling Prep API key.
        exchange (str, optional): Stock exchange to filter by. Defaults to "NASDAQ".

    Returns:
        pd.DataFrame: A DataFrame containing realtime quote data for the requested ticker(s).
    """
    if isinstance(tickers, list):
        tickers = ",".join(tickers)
...
def get_realtime_data(api_key, exchange="NASDAQ"):
    ...
    params = {"short": "false", "exchange": exchange, "apikey": api_key}
...
if __name__ == "__main__":
    ...
    intraday_df = get_intraday_data(
        selected_tickers, from_date, to_date, interval, api_key
    )
    ...
    MY_TICKERS = tickers  # Use fetched S&P 500 tickers
